...
#     resolution = np.array([2,2,2])
    name = filelist[id]
    #add by syuu start
    #name_seg = '0' * (3 - len(str(name))) + str(name)
    # add by syuu end
...
def preprocess_luna():
    ...
    #add by syuu start
    #[os.rename(os.path.join(luna_data, f), os.path.join(luna_data, '0' * (7 - len(str(f))) + str(f))) for f in os.listdir(luna_data) if len(f)<7]
    # add by syuu end
    if not os.path.exists(finished_flag):
